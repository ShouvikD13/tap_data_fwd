...
int i_ord_mkt_stts;
int i_ext_mkt_stts;int i_exr_mkt_stts;
int i_pmp_stts;
int i_upd_stts;
...
int fn_do_reco ( char c_reco_mode,
                 char *c_tm_stmp,
                 char *c_ServiceName,
                 char *c_err_msg );

int fn_prcs_reco( char c_reco_mode,			/***	Ver 1.6	***/
                  char *c_tm_stmp,
									long li_stream_cnt,
                  long li_stream_no,
                  char *c_ServiceName,
...
int fn_get_nxt_rec ( char *c_ServiceName,
                     char *c_err_msg );
...
void fn_print_details(int i_index,
											char *c_ServiceName,
                      char *c_err_msg);
...
int fn_get_ext_cnt( struct vw_contract *ptr_st_cnt,
...
int fn_flr_ord_plc(char *c_ordr_rfrnc,
                   ...
                   char *c_ServiceName,
                   char *c_err_msg);
void CLN_PACK_CLNT ( int argc, char *argv[] )
{
		int i_trnsctn;	
		int i_ch_val;
		int i_lmt_counter = 1;
				...
				{	
				  ...
				  EXEC SQL
					...
					(
					 ...
					 :c_brkr_id,
					 'T',
					 'Forwarding started for Pipe ID '||:sql_c_pipe_id,
					 sysdate
					);
                     ...
                     char *c_err_msg )
{
  ...
  char c_prgm_flg='\0';  /** Ver 2.7 **/
	char c_pan_no[11];				 /** Ver 3.6 **/
	char c_lst_act_ref[22]; /*** Ver 3.8 ***/
	char c_esp_id [51]; /*** Ver 4.1 ***/
	char c_algo_id[51]; /*** Ver 4.1 ***/
	char c_source_flg='\0';/*** Ver 4.1 ***/
              ...
              )
{
  ...
  if ( SQLCODE != 0 )
  {
	...
	if ( c_reco_mode	!= 'N' )
	{
  	if( li_stream_no  ==  0 )
  	{
    	i_rtrn_val  = fn_prcs_reco( c_reco_mode,
      	                          c_tm_stmp,
																	li_stream_cnt,
          	                      li_stream_no,
            	                    c_ServiceName,
...
int fn_prcs_reco( char c_reco_mode,
                  char *c_tm_stmp,
									long li_stream_cnt,
                  long li_stream_no,
                  char *c_ServiceName,
    ...
    if ( i_ord_bod_stts == 1 || i_trd_bod_stts == 1 )
    {
  	...
  	if ( i_rec_stts == 1)
  	{
    	...
    	EXEC SQL
      	...
      	and   fxb_pipe_id    = :sql_c_pipe_id
      	and   fxb_mod_trd_dt = :sql_c_nxt_trd_date
      	and   fxb_ack_tm <= to_date ( :sql_c_tm_stmp,'dd-mon-yyyy-hh24:mi:ss')
    	...
    	if ( SQLCODE == -1405 )
    	{
      	d_strt_tm_o =  0;
			...
			EXEC SQL
     		...
     		and   fod_pipe_id    = :sql_c_pipe_id
     		and   ftd_trd_dt     > :sql_c_nxt_trd_date
     		and   ftd_trd_dt     <= to_date ( :sql_c_tm_stmp,'dd-mon-yyyy-hh24:mi:ss' )
     		and   ftd_stream_no  =  :li_stream_cnt;			 /***  Ver 1.8 li_stream_no Is Replaced By li_stream_cnt ***/
...
void fn_print_details(int i_index,
                      ...
                      char *c_err_msg)
{
	if((	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_spdoe_reqres.\
				st_hdr.si_transaction_code == SP_BOARD_LOT_IN ) || 
			(	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_spdoe_reqres.\
				st_hdr.si_transaction_code == TWOL_BOARD_LOT_IN	) || 
			( st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_spdoe_reqres.\
				st_hdr.si_transaction_code == THRL_BOARD_LOT_IN ))
	{
		if( DEBUG_MSG_LVL_1 ) 
		{
	...
	else if((	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_oe_res.\
					 	st_hdr.si_transaction_code == BOARD_LOT_IN ) || 
					(	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_oe_res.\
						st_hdr.si_transaction_code == ORDER_MOD_IN ) || 
					(	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_oe_res.\
						st_hdr.si_transaction_code == ORDER_CANCEL_IN ))
	{
		if( DEBUG_MSG_LVL_1 ) 
		{
	...
	else if((	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_expl_reqres.\
						st_hdr.si_transaction_code == EX_PL_ENTRY_IN	) || 
					(	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_expl_reqres.\
						st_hdr.si_transaction_code == EX_PL_MOD_IN	) || 
					(	st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.st_expl_reqres.\
						st_hdr.si_transaction_code == EX_PL_MOD_IN	))
	{		
		if( DEBUG_MSG_LVL_1 ) 
		{
			fn_userlog(	c_ServiceName,"Exercise order reference is :%s: FSP sequence is :%d:",
								st_sndq_packets[i_index].st_exch_msg_data.st_exch_sndmsg.\
								st_expl_reqres.st_ex_pl_data.c_remarks,
								st_sndq_packets[i_index].st_exch_msg_data.st_net_header.i_seq_num);	
		}
	}
	else
      ...
      SELECT  fod_clm_mtch_accnt,
              ...
              fod_xchng_cd,
               /*** decode(:ptr_st_orderbook->c_oprn_typ,'S',
                      (decode(fod_prdct_typ,'P','F',fod_prdct_typ)),fod_prdct_typ),  ***Commented in ver 2.1 ***/
							/*** decode(:ptr_st_orderbook->c_oprn_typ,'S',(decode(fod_prdct_typ,'P','F','U','F',fod_prdct_typ)),fod_prdct_typ),  *** Ver 2.1 *** Commented in ver 2.3 ***/
  ...
  if(c_sprd_ord_ind!=SPREAD_ORDER && 
	...
	{ 
	...
	else
 	{
 			...
 			UPDATE FXB_FO_XCHNG_BOOK 
			SET FXB_PLCD_STTS='M'
 			WHERE FXB_ORDR_RFRNC IN (SELECT FSD_ORDR_RFRNC 
														FROM FSD_FO_SPRD_DTLS
