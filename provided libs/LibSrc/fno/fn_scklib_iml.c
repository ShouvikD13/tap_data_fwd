/******************************************************************************/
/*  Program         : fn_scklib_iml.c                                         */
/*                                                                            */
/*  Description     : Library functions for Socket handling operations        */
/*                                                                            */
/*  Log             :                                                         */
/*                                                                            */
/*  Ver 1.0   13-Apr-2010  Initial Release (BSE-FNO)		                              */
/******************************************************************************/
/****  C header   ********/
#include <sys/types.h>
...
#include <fo_exch_comnN.h>  /* VER TOL : TUX on LINUX */
...
/* TCP_NODELAYACK changed to TCP_NODELAY in VER TOL */
if( setsockopt(i_tmp_sck_id, IPPROTO_TCP, TCP_NODELAY, &i_setval, ud_optlen ) == -1 )   
{
...
/*       -1 - Failure                                                         */
/******************************************************************************/
int fn_read_bse_sck(int i_sck_id,
                    void *ptr_v_data,
                    long int li_timeout,
                    char *c_ServiceName,
                    char *c_err_msg)
{
  ...
  struct st_bfo_header		  st_msg_hdr;  
  ...
  /*First read the header*/
 	if (DEBUG_MSG_LVL_3)
	{
	fn_userlog(c_ServiceName,"Before calling fn_readn in fn_read_bse_sck: Val of i_sck_id=<%d> and sizeof(st_msg_hdr)=<%ld> ", i_sck_id), sizeof(st_msg_hdr);
	}
  ...
  if (DEBUG_MSG_LVL_5)
  {
  fn_userlog(c_ServiceName, "Val of i_ch_val after fn_readln: <%d>", i_ch_val);
  fn_userlog(c_ServiceName, "Val of st_msg_hdr.l_slot_no  after fn_readln: <%ld>", st_msg_hdr.l_slot_no);
  fn_userlog(c_ServiceName, "Val of st_msg_hdr.l_message_length  after fn_readln: <%ld>", st_msg_hdr.l_message_length);
  }
...
/********************************************************/
  ...
  if (DEBUG_MSG_LVL_5)
  {
   fn_userlog(c_ServiceName, "IML_lib:After reading the header in scklib"); 
  }
  /*Read the remainder packet*/
	st_msg_hdr.l_message_length = fn_swap_long(st_msg_hdr.l_message_length);

  li_read_len = st_msg_hdr.l_message_length ;


	if (DEBUG_MSG_LVL_5)
	{
	 fn_userlog(c_ServiceName, "Val of li_read_len after difference chk <%ld>", li_read_len); 
	}
  ...
  if(li_read_len > 0)
  {
	  if (DEBUG_MSG_LVL_5)
	  {
	   fn_userlog(c_ServiceName, "IML_lib:Reading rest of struct other than header in scklib");
	  }
		...
		if(li_read_len > (MAX_SCK_MSG - sizeof(st_msg_hdr)))
    {
      li_read_len = MAX_SCK_MSG - sizeof(st_msg_hdr);
	    if (DEBUG_MSG_LVL_5)
	    {
	    fn_userlog(c_ServiceName, "Before call to fn_readn ..val of sizeof(st_msg_hdr) <%ld> and MAX_SCK_MSG<%ld>", sizeof(st_msg_hdr), MAX_SCK_MSG );
	    }
  	}
		if (DEBUG_MSG_LVL_5)
		{
		fn_userlog(c_ServiceName, "Before call to fn_readn ..val of li_read_len<%ld> and i_sck_id<%ld>",li_read_len, i_sck_id );
		}
		...
		if (DEBUG_MSG_LVL_5)
		{
		fn_userlog(c_ServiceName, "Val of i_ch_val after fn_readn is <%d>", i_ch_val);
  	fn_userlog(c_ServiceName, "IML_lib:After Reading rest of struct other than header in scklib on c_buffer");
	}
      ...
      if(li_timeout > 0)
      {
	     fn_userlog(c_ServiceName, "Before fn_set_tmr_sigdisp ----1 ");
        fn_set_tmr_sigdisp(TM_0, c_ServiceName, c_err_msg);
	     fn_userlog(c_ServiceName, "After fn_set_tmr_sigdisp ----1 ");

      }
      fn_userlog(c_ServiceName, "After timeout check ----1 ");
      return -1;
  	}
   ...
   /*Copy the message data into the return buffer*/
  	if (DEBUG_MSG_LVL_5)
	{
	fn_userlog(c_ServiceName, "IML_lib:Before Copying data in scklib on c_buffer");
	}
  	memcpy((char *)ptr_v_data + sizeof(st_msg_hdr), c_buffer, li_read_len);
  	if (DEBUG_MSG_LVL_5)
	{
	fn_userlog(c_ServiceName, "IML_lib:After Copying data in scklib on c_buffer");
	}
             ...
             char *c_err_msg)
{
  ...
  if (DEBUG_MSG_LVL_5)
  {
  fn_userlog(c_ServiceName, "Val of i_sck_id in fn_readn fn b4 read <%d> and li_left <%d>", i_sck_id, li_left);
  }
  while((li_left > 0) && (i_break_status_sck == DONT_EXIT))
  {
    ...
    if (DEBUG_MSG_LVL_5)
    {
    fn_userlog(c_ServiceName, "Val of i_read = :%d:, li_left = :%ld: before each read fn in loop", i_read, li_left);
    }
    ...
    if (DEBUG_MSG_LVL_5)
    {
    fn_userlog(c_ServiceName, "BSE:bytes read from socket = :%d:",i_read);
    }
    li_left = li_left - i_read;
    if (DEBUG_MSG_LVL_5)
    {
    fn_userlog(c_ServiceName, "After BSE:bytes read from socket = :%d:",li_left);
    }
              ...
              char *c_err_msg)
{
  ...
  while((li_left > 0) && (i_break_status_sck == DONT_EXIT))
  {
    ...
    if(DEBUG_MSG_LVL_3)
    {
    fn_userlog(c_ServiceName, "Written :%ld: bytes on socket in fn_written fn with socket id is [%d] and li_left<%ld>", li_written, i_sck_id, li_left);
    }
  ...
  {
    fn_errlog(c_ServiceName, "L31065", "User Signal is caught", c_err_msg);
    return -1;
  }
...
{
  int       i_read;
  socklen_t li_addrss_len;    /* Added in VER TOL */
  /* COMMENTED IN VER TOL * unsigned long li_addrss_len; */
  char      *ptr_v_tmp_data;
                          ...
                          &li_addrss_len)) == -1)
  {
    if (errno == EINTR)
    {
      ...
      {
        i_sig_intrpt_trp = 0;
        fn_errlog(c_ServiceName, "L31100", "Timer expired", c_err_msg);
      }
      else if(i_break_status_sck == EXIT)
      {
        fn_errlog(c_ServiceName, "L31105", "Signal caught", c_err_msg);
      }
      else
      {
        fn_errlog(c_ServiceName, "L31110", UNXMSG, c_err_msg);
      }
  ...
  {
    fn_errlog(c_ServiceName, "L31120", "Socket in other end closed", c_err_msg);
    if(li_timeout > 0)
    {
      fn_set_tmr_sigdisp(TM_0, c_ServiceName, c_err_msg);
    }
...
void fn_timearray_to_long(char *c_tm_stmp,
                        long int *ptr_li_dest)   /* Renamed fn_timearr_to_long to fn_timearray_to_long in Ver TOL */
{
  int  i_srcindex;
  int  i_arrayindex;
  char c_tmp;
                       ...
                       char *c_err_msg )
{
	...
	if (st_op_hdr.li_msg_typ != 0)
  {
    ...
    {
      fn_errlog(c_ServiceName, "L31125", "Message mismatch", c_err_msg);
      fn_set_tmr_sigdisp ( TM_0,
                          c_ServiceName,
                          c_err_msg       );
      return -1;
                     ...
                     char *c_err_msg)
{
  ...
  {
    fn_errlog(c_ServiceName, "L31130", UNXMSG, c_err_msg);
    return -1;
  }
  ...
  if ( bind ( i_tmp_sckid,
              ...
              sizeof(struct sockaddr_in)  ) == -1 )
  {
    fn_errlog(c_ServiceName, "L31135", UNXMSG, c_err_msg);
    return -1;
  }
                   ...
                   char *c_err_msg)
{
  ...
  fn_errlog(c_ServiceName, "L31150", "Interrupt caught", c_err_msg);
  return -1;
}
              ...
              )
{
  ...
  if( i_res == -1)
  {
    fn_errlog(c_ServiceName, "L31155", UNXMSG, c_errmsg);
    return -1;
  }
