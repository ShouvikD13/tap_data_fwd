/******************************************************************************/
/*  version 1.0 - 01-Mar-2010 New Release   Shailesh Hinge.                   */
/*  version 1.1 - 11-Jun-2012 Active Fallback Provision   Shailesh Hinge.     */
/*  version 1.2 - 08-Oct-2013 Fallback Fallback Provision Mahesh Shinde.      */
/******************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <userlog.h>
...
int i_span_sck_id;
int i_sck_indctr;					/*Ver 1.1 */
char sql_c_pipe_id [ 3 ];
char c_primary_ip[20];	  /*Ver 1.1 */
char c_backup_ip[20];			/*Ver 1.1 */
char c_third_ip[20];			/*Ver 1.2 */
char c_span_srvr_id[20];	/*Ver 1.2 */
long li_port;							/*Ver 1.1 */
...
int fn_crt_sck(char *c_ServiceName, char *c_err_msg);		/*Ver 1.1 */
...
int tpsvrinit ( int argc, char * argv[] )
{
		...
		int   i_ch_val;
		int   i_primary_fallback_indctr;     /**Ver 1.1 **/
		char  *ptr_c_tmp;
	...
	/*	while ( strcmp ( argv[i_ch_val], "--" ) != 0 )
		...
		if ( tpopen( ) == -1 )
  	{
		...
		/*** Initialize Process Space  ***/
	 	...
	 	i_ch_val = fn_init_prcs_spc( "SFO_SPAN_MRGN",
                                 ptr_c_fileName,
                                 "SFO_SPAN_MRGN" );
  	if (i_ch_val == -1)
  	{
		...
		strcpy(c_primary_ip, ptr_c_ip_addr);								/*Ver 1.1 */
		...
		strcpy(c_backup_ip, ptr_c_ip_addr);								/*Ver 1.1 */
...
int fn_crt_clnt_sck(char *ptr_c_ip_addr,
                    long  i_port_no,       /* Ver TOL : TUX on LINUX -- Int chnaged to Long, as per function declaration at top */
                    int  *ptr_i_sck_id,
                    char *c_ServiceName,
                    char *c_err_msg)
{
...
/* Ver 1.1 added */
  ...
  /* TCP_NODELAYACK changed to TCP_NODELAY in Ver TOL : TUX on LINUX */
  i_setval = 1;
  if( fn_setsockopt ( i_tmp_sck_id, IPPROTO_TCP, TCP_NODELAY, &i_setval, ud_optlen ) == -1 )
  {
