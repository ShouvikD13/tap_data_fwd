...
extern char c_rqst_for_open_ordrs;
                      ...
                      char *c_err_msg)
{
  ...
  i_cond_snd_rcv_cntrl = LOGON_REQ_SENT; // #define LOGON_REQ_SENT 2
                              ...
                              char *c_err_msg)
{
  ...
  if(  st_sys_info_dat->st_hdr.c_alpha_char[0] =='	')
  {
    fn_userlog(c_ServiceName,"Inside Stream No. Check");
    li_stream_no  = 0;
    fn_userlog(c_ServiceName," li_stream_no Is :%ld:",li_stream_no);
  }
  else
  {
    li_stream_no = st_sys_info_dat->st_hdr.c_alpha_char[0];
...
int fn_validate_tap(int *i_exg_seq, char *c_wholeptr_data, char *c_ServiceName)
{
  ...
  /***** commented in Ver 1.7 ***
  if ( (*i_exg_seq) != st_net_header.i_seq_num)
  {
    fn_userlog(c_ServiceName,"Error in matching sequence number ");
    return -1;
  } ***/
...
int fn_exch_exception_dwld(struct st_brd_exch_exception *st_brd_exch_exception_msg,
                           char *c_xchng_cd,
                           char *c_pipe_id,
                           char *c_ServiceName,
                           char *c_errmsg)
