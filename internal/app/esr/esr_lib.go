...
func (ESRM *ESRManger) Fn_chng_exp_passwd_req(db *sql.DB, pipe_id sql_c_pipe_id, new_gen_passwd [9]byte) error {
	...
	newPasswordStr, err := ESRM.PUM.FngetPassword(string(curntPasswd[:]), CRNT_PSSWD_LEN)
	if err != nil {
	...
	_, err := tm.Tran.Exec(updateStmt, sqlCPipeID, sqlCXchngCd)
	if err != nil {
	...
	if sign_on_resp.C_broker_status == 'A' {
		...
		SET OPM_LST_PSWD_CHG_DT = TO_TIMESTAMP(?, 'DD-Mon-YYYY HH24:MI:SS'),
			OPM_LOGIN_STTS = 1
		WHERE OPM_PIPE_ID = ?`
		_, err = tm.Tran.Exec(updateStmt, string(sqlPasswdLstUpdtDt[:]), sqlCPipeID)
		if err != nil {
